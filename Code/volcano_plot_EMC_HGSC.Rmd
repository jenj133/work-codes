...
```{r}
dat <- read.csv("../Data/AllComparisons_HGSC_EMC.csv", header = TRUE, stringsAsFactors = FALSE)
```
...
plot_ass <- volcanol_plot+theme_hc() + 
  ...
  ggtitle("Differentially expressed proteins in ENOC compared to HGSC") +
  xlab("Fold change")+
  ylab("-log10(p.adj)")+
...
Print PDF
```{r}
pdf("volcano_ENOC_HGSC.pdf", width = 8, height= 6)
plot_ass
dev.off()
```